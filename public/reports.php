...
<body class="pt-24 bg-gray-50">
    ...
    <div class="container mx-auto p-6">
        <div class="flex justify-between items-center mb-6">
            ...
            <button onclick="openAddModal()" class="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-700 transition">
                ...
                <tbody class="divide-y divide-gray-200">
                    <?php foreach ($reports as $report): ?>
                        <tr class="border-b hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-4 py-2 text-sm max-w-xs truncate" title="<?= htmlspecialchars($report['report_desc']) ?>">
                            ...
                            <td class="px-4 py-2 text-xs text-gray-500">
                                <?php if ($report['updated_by']): ?>
                                    <span class="font-medium text-gray-700">By <?= htmlspecialchars($report['updater_name']) ?></span><br>
                            ...
                            <?php if ($isAdmin): ?>
                                <td class="px-4 py-2">
                                    <button class="edit-report-btn bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-sm transition-colors"
       ...
       <div id="addReportModal" class="hidden fixed inset-0 z-[150] flex items-center justify-center p-6 backdrop-blur-sm">
    <div class="absolute inset-0 bg-black/40" onclick="closeAddModal()"></div>
    
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden z-10 flex flex-col min-h-[600px]">
        ...
        <form action="../controllers/add_report.php" method="POST" enctype="multipart/form-data" class="p-6 space-y-6 flex-grow overflow-y-auto">
            
            <div class="space-y-5">
                <div>
                    <label for="cat_id" class="block text-sm font-bold text-gray-700 mb-1.5">Category</label>
                    <select name="cat_id" id="cat_id" class="w-full border border-gray-300 rounded-xl p-4 focus:ring-4 focus:ring-blue-500 outline-none text-sm transition-all" required>
                ...
                <div>
                    <label for="mod_id" class="block text-sm font-bold text-gray-700 mb-1.5">Module</label>
                    <select name="mod_id" id="mod_id" class="w-full border border-gray-300 rounded-xl p-4 focus:ring-4 focus:ring-blue-500 outline-none text-sm transition-all" required>
                ...
                <div>
                    <label for="sev_id" class="block text-sm font-bold text-gray-700 mb-1.5">Severity Level</label>
                    <select name="sev_id" id="sev_id" class="w-full border border-gray-300 rounded-xl p-4 focus:ring-4 focus:ring-blue-500 outline-none text-sm transition-all" required>
                ...
                <div>
                    <label for="rep_desc" class="block text-sm font-bold text-gray-700 mb-1.5">Describe What Happened</label>
                    <textarea name="rep_desc" id="rep_desc" rows="5" class="w-full border border-gray-300 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 outline-none text-sm transition-all resize-none" placeholder="Explain the issue..." required></textarea>
                </div>
            </div>

            <div class="flex justify-end gap-3 pt-6">
                <button type="button" onclick="closeAddModal()" class="flex-1 px-4 py-2.5 text-sm font-semibold text-gray-500 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors">
                    Cancel
                </button>
                <button type="submit" class="flex-1 px-4 py-2.5 text-sm bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 shadow-md transition-all active:scale-95">
                    Submit Report
                </button>
            </div>
        </form>
...
<div id="editModal" class="hidden fixed inset-0 z-[150] flex items-center justify-center p-6 backdrop-blur-sm">
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden z-10 flex flex-col min-h-[600px]">
        ...
        <form id="editForm" action="../controllers/edit_reports.php" method="POST" class="p-6 space-y-6 flex-grow overflow-y-auto">
            <input type="hidden" name="report_id" id="edit_report_id">
            
            <div class="space-y-5">
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1.5">Category</label>
                    <select name="cat_id" id="edit_cat_id" class="w-full border border-gray-300 rounded-xl p-4 focus:ring-4 focus:ring-blue-500 outline-none text-sm">
                        <option value="other">Other</option>
                ...
                <div>
                    ...
                    <select name="mod_id" id="edit_mod_id" class="w-full border border-gray-300 rounded-xl p-4 focus:ring-4 focus:ring-blue-500 outline-none text-sm">
                        <option value="other">Other</option>
                ...
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1.5">Severity</label>
                    <select name="sev_id" id="edit_sev_id" class="w-full border border-gray-300 rounded-xl p-4 focus:ring-4 focus:ring-blue-500 outline-none text-sm">
                ...
                <div>
                    ...
                    <textarea name="report_desc" id="edit_desc" rows="5" class="w-full border border-gray-300 rounded-xl p-3 focus:ring-2 focus:ring-blue-500 outline-none text-sm resize-none"></textarea>
                </div>
            </div>
            
            <div class="flex justify-end gap-3 pt-6">
                <button type="button" onclick="closeModal()" class="flex-1 px-4 py-2.5 text-sm font-semibold text-gray-500 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors">
                    Cancel
                </button>
                <button type="submit" class="flex-1 px-4 py-2.5 text-sm bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 shadow-md transition-all active:scale-95">
                    Save Changes
